#include "block-helper.hpp"

BlockFacing BlockHelper::getFacingFromVector(vec3 v) {
    v = glm::normalize(v);
    if(v.z == 1) {
...
quat BlockHelper::getRotationFromFacing(BlockFacing blockFacing) {
    switch(blockFacing) {
...
void BlockHelper::addMesh(MeshData<ConstructionVertex>& meshData,ivec3 position,quat rotation,const MeshData<Vertex>& blockMeshData,TextureID texture) {
    int indexOffset = meshData.vertices.size();
...
ivec3 BlockHelper::rotateByFacing(ivec3 vec,BlockFacing facing) {
...
void BlockHelper::addConnectedBlock(Construction* construction,MeshData<ConstructionVertex>& meshData,ivec3 position,TextureID texture) {
