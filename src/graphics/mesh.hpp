    ...
    public:
        ...
        void loadFromFile(string path) {
            ...
            if constexpr(!std::is_same_v<V,Vertex>) {
                Debug::warn("Trying to load model with unsupported vertex type");
                return;
            }
            ...
            for(string line : FileHelper::readToStrings(path)) {
                if(line.length() > 2) {
                    ...
                    if(lineSegments[0] == "f") {
                        ...
                        std::unordered_map<V,int> vertexMap;
        ...
        void createBuffers(Vulkan* vulkan) {
            ...
            meshBuffer = vulkan->createMeshBuffers<V>(meshData.vertices,meshData.indices);
            buffersLoaded = true;
