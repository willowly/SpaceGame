...
public:
    ...
    struct EventBlockPlaced
    {
        ...
        BlockPaletteEntry blockEntry;
        ivec3 position;
    };

    Event<EventBlockPlaced> onBlockPlaced;

    struct EventBlockBroken
    {
        Construction *construction = nullptr;
        ivec3 position;
    };
    ...
    void placeBlockNoUpdate(ivec3 location, Block &block, BlockStorage storage, bool attached)
    {
        ...
        block.onLoad(this, location, entry.storage);
    ...
    void calculateBreakup(World *world, ivec3 location)
    {
        ...
        {
            if (constructions[i] != nullptr)
            { // regenerate all at the end
    ...
    {
        if (location.x > boundsMax.x)
            return false;
        if (location.y > boundsMax.y)
            return false;
        if (location.z > boundsMax.z)
            return false;

        if (location.x < boundsMin.x)
            return false;
        if (location.y < boundsMin.y)
            return false;
        if (location.z < boundsMin.z)
            return false;
        return true;
    }
